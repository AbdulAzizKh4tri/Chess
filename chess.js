...
class Game{
    constructor(){
        this.board = create2DArray(8,8,()=>{return new Cell()})
    ...
    renderBoard(){
        ...
        let end = 1
        let incr = -1
        let condition = (s,e) => { return s >= e}
        let jstart = 1
        let jend = 8
        let jincr = 1
        let jcondition = (s,e) => {return s <= e}
        if((TURN_BOARD && TURN == BLACK) || PLAYER == BLACK){
            start = 1
            end = 8
            incr = 1
            condition = (s,e) => { return s <= e}
            jstart = 8
            jend = 1
            jincr = -1
            jcondition = (s,e) => { return s >= e}
        }
        ...
        for(let i = 0;i < 8;i++){
            for(let j = 0;j < 8;j++){
                ...
                if(curr.piece){
                    ...
                    img.src = `./themes/${THEME}/${curr.piece.color}/${curr.piece.img}.png`
                    img.classList += " piece "
        ...
        for(let i = 0;i < 8;i++){
            for(let j = 0;j < 8;j++){
                ...
                if(curr && curr.color == TURN && (PLAYER == TURN || true)){
                    ...
                    button.onclick = () => {this.showMoves(j,i)}
                }else{
                    button.disabled = true
                }
            }
    ...
    showMoves(col,row){
        ...
        for(let move of moves){
            let [mcol,mrow] = move
            let movecell = document.getElementById(translateToChars(mcol,mrow))
    ...
    getPieceMoves(piece){
        ...
        if(piece instanceof(Pawn)){
            ...
            if(piece.color==WHITE && row == 1 || piece.color == BLACK && row == 6){
                ...
                if(this.board[front][col].piece == null && this.board[doublefront][col].piece == null){
            ...
            if(!piece.has_moved && !this.board[row][col].in_danger){
                ...
                if(rook1 && !rook1.has_moved && 
                    !this_row[col-1].piece && !this_row[col-2].piece && !this_row[col-3].piece &&
                    !this_row[col-1].in_sight && !this_row[col-2].in_sight && !this_row[col-3].in_sight){
                    moves.push([col-2,row])
                }
                if(rook2 && !rook2.has_moved &&
    ...
    getPossibleMoves(piece){
        ...
        for(let move of moves){
            ...
            if(this.board[mrow][mcol].piece && this.board[mrow][mcol].piece.color==piece.color) continue
    ...
    makeMove(piece,next_col,next_row,forward = true){
        ...
        if(forward && this.isPromotion(piece,next_row)){
            ...
            while(!selected){
                let x = prompt("Promote to? [(Q)ueen,(K)night, (B)ishop, (R)ook")
                switch(x){
                    case "Q": case "q":
    ...
    isEnpassant(piece,new_row,new_col,prev_col){
        if(piece instanceof(Pawn) && new_col != prev_col && this.board[new_row][new_col].piece == null){
            return true
        }
        return false
    }
    ...
    undoMove(){
        ...
        if(move[4] == "enpassant"){
            let pawn = new Pawn(otherColor(piece.color),translateToChars(next_col,next_row+behind))
        ...
        if(move[4] == "promotion"){
            let pawn = new Pawn(piece.color,translateToChars(next_col,next_row))
            new_cell.piece = pawn
        ...
        if(move[4] == "castle_H"){
            ...
            this.board[prev_row][7].piece =  new Rook(piece.color,translateToChars(7,prev_row))
            TURN = piece.color
            if(TURN_BOARD){
                this.renderBoard()
            }
        ...
        if(move[4] == "castle_A"){
            ...
            this.board[prev_row][0].piece =  new Rook(piece.color,translateToChars(0,prev_row))
            TURN = piece.color
            if(TURN_BOARD){
                this.renderBoard()
            }
...
function setTheme(){
    ...
    let path =`themes/${THEME}/theme.json`
    fetch(path) 
    ...
    .then((data) => {
        ...
        if(BACKGROUND_IMAGE){
            document.body.style.backgroundImage = `url('themes/${THEME}/${BACKGROUND_IMAGE}')`
        }else{
...
function load(){
    ...
    game.renderPieces()
    game.enableMovables()
    document.getElementById("undo").onclick = () => {game.undoMove()}
    document.getElementById("unundo").onclick = () => {game.undoMove();game.undoMove()}
    document.getElementById("redo").onclick = () => {game.redoMove()}
    document.getElementById("reredo").onclick = () => {game.redoMove();game.redoMove()}
